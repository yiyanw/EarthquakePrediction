...
def cacu_features(df: pd.DataFrame, window: int, step: int, tag: str, eqData: pd.DataFrame, flag: str) -> pd.DataFrame:
    ...
    for feature in ['average', 'diff_1']:
        ...
        res_df = pd.merge(res_df, tmp[
            ['Day', f'{feature}_day_max_mean', f'{feature}_day_min_mean', f'{feature}_day_mean_max',
             f'{feature}_day_mean_min']], on='Day', how='left')
...
if __name__ == "__main__":
    ...
    area_groups = [
        ...
        {'id': set(
            [128, 129, 19, 26, 159, 167, 170, 182, 310, 184, 188, 189, 191, 197, 201, 204, 88, 90, 91, 93, 94, 221, 223,
             98, 107, 235, 236, 252, 250, 124, 125]), 'range': [30, 34, 101, 104]},
        ...
        {'id': set(
            [256, 130, 132, 147, 148, 149, 151, 153, 32, 33, 35, 60195, 38, 39, 41, 302, 304, 177, 305, 307, 181, 309,
             314, 315, 316, 317, 319, 320, 193, 322, 200, 73, 329, 75, 333, 78, 334, 84, 87, 60251, 96, 225, 101, 229,
             105, 109, 40047, 240, 247, 120, 254, 255]), 'range': [26, 30, 101, 104]},
    ...
    for i, area in enumerate(area_groups):
        ID_list = area['id']  # 站点id
        range_list = area['range']  # 经纬度
        ...
        for flag in ['train', 'valid']:
            ...
            _df_magn = local_magn_data[
                (local_magn_data['TimeStamp'] >= start_stamp) & (local_magn_data['TimeStamp'] < end_stamp)]
            _df_sound = local_sound_data[
                (local_sound_data['TimeStamp'] >= start_stamp) & (local_sound_data['TimeStamp'] < end_stamp)]
